...
Server.prototype.serveDir = function(pathname, req, res, finish) {
  ...
  fs.stat(htmlIndex, function(e, stat) {
    if (!e) {
      ...
      if (
        originalPathname.length &&
        originalPathname.charAt(originalPathname.length - 1) !== "/"
      ) {
...
Server.prototype.serveFile = function(pathname, status, headers, req, res) {
  ...
  fs.stat(pathname, function(e, stat) {
    ...
    that.respond(null, status, headers, [pathname], stat, req, res, function(
      status,
      headers
    ) {
...
Server.prototype.servePath = function(
  pathname,
  status,
  headers,
  req,
...
Server.prototype.serve = function(req, res, callback) {
  ...
  process.nextTick(function() {
    that
      .servePath(pathname, 200, {}, req, res, finish)
 ...
 */
Server.prototype.respondGzip = function(
  pathname,
  status,
  contentType,
  _headers,
...
Server.prototype.respondNoGzip = function(
  pathname,
  status,
  contentType,
  _headers,
...
Server.prototype.respondNoGzip1 = function(
  pathname,
  status,
  contentType,
  _headers,
  ...
  if (files.length == 1 && byteRange.valid) {
    if (byteRange.to < length) {
      ...
      headers["Content-Range"] =
        "bytes " + byteRange.from + "-" + byteRange.to + "/" + stat.size;
    } else {
...
Server.prototype.respond = function(
  pathname,
  status,
  _headers,
  files,
...
Server.prototype.stream = function(
  pathname,
  files,
  buffer,
  startByte,
  ...
  (function streamFile(files, offset) {
    ...
    if (file) {
      file =
        path.resolve(file) === path.normalize(file)
          ? file
    ...
    fs.readFile(files[0], "utf-8", function(e, s) {
      ...
      while (i < s.length) {
        if (s.substr(i, 15) == "// scope:server") {
          ...
          i += 15;
        } else if (s.substr(i, 18) == "/* scope:server */") {
          i += 18;
