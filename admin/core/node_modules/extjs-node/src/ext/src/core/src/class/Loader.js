...
Ext.Loader = new (function() {
  ...
  Ext.apply(Loader, {
    ...
    setConfig: function(name, value) {
      if (Ext.isObject(name) && arguments.length === 1) {
        ...
        Loader.config[name] = Ext.isObject(value)
          ? Ext.merge(Loader.config[name], value)
          : value;
      }
    ...
    getPath: function(className) {
      ...
      return (
        path.replace(slashDotSlashRe, "/") +
        className.replace(dotRe, "/") +
        ".js"
      );
    },
  ...
  Ext.apply(Loader, {
      ...
      var script = document.createElement("script"),
        ...
        onLoadFn = function() {
          if (!dispatched) {
            ...
            Loader.cleanupScriptElement(
              script,
              config.preserveScripts === false,
    ...
    removeScriptElement: function(url) {
      if (scriptElements[url]) {
        Loader.cleanupScriptElement(
          scriptElements[url],
          true,
          !!Loader.getConfig("garbageCollect")
        );
    ...
    loadScript: function(options) {
      var config = Loader.getConfig(),
        ...
        onScriptError = function() {
          ...
          if (onError) {
            onError.call(
              scope,
              "Failed loading '" + url + "', please verify that the file exists"
            );
          }
      ...
      src = config.disableCaching
        ? url + "?" + config.disableCachingParam + "=" + Ext.Date.now()
        : url;

      scriptElements[url] = Loader.injectScriptElement(
        src,
        onScriptLoad,
        onScriptError
      );
      ...
      var config = Loader.getConfig(),
        noCacheUrl =
          ...
          (config.disableCaching
            ? "?" + config.disableCachingParam + "=" + Ext.Date.now()
            : ""),
        ...
        if (
          ...
          onError.call(
            ...
            "Failed loading synchronously via XHR: '" +
              url +
              "'; It's likely that the file is either " +
    ...
    onFileLoaded: function(className, filePath) {
      ...
      //<debug>
      if (
        !Loader.syncModeEnabled &&
        Loader.numPendingFiles === 0 &&
        Loader.isLoading &&
        ...
        missingClasses = Ext.Array.filter(
          Ext.Array.unique(missingClasses),
          function(item) {
    ...
    historyPush: function(className) {
      if (
        className &&
        isClassFileLoaded.hasOwnProperty(className) &&
        !isInHistory[className]
      ) {
  ...
  Ext.disableCacheBuster = function(disable, path) {
    ...
    document.cookie =
      "ext-cache=1; expires=" + date + "; path=" + (path || "/");
  };
      ...
      Ext.apply(Loader, {
        ...
        loadScriptFile: function(
          ...
          try {
            ...
            if (process.env.MODE == "PROD") console.error(e);
            else this.requireSharedFile(filePath);
          }
          onLoad.call(scope);
        },

        requireSharedFile: function(filePath) {
          if (!Loader.config.sharedPath) return;
          for (var i = 0; i < Loader.config.sharedPath.length; i++) {
            if (Loader.config.sharedPath[i].src.test(filePath)) {
              this.requireFile(
                filePath.replace(
                  Loader.config.sharedPath[i].src,
                  Loader.config.sharedPath[i].dst
                )
              );
              return;
            }
        ...
        requireFile: function(file) {
          ...
          while (i < s.length) {
            if (s.substr(i, 15) == "// scope:client") {
              ...
              i += 15;
            } else if (s.substr(i, 18) == "/* scope:client */") {
              i += 18;
  ...
  Class.registerPreprocessor(
    "loader",
    function(cls, data, hooks, continueFn) {
      var me = this,
        dependencies = [],
        dependency,
