Ext.define("Core.data.actions.Read", {
  ...
  buildLimits: function(params, callback) {
    ...
    if (isNaN(limit)) limit = this.maxLimit;
  ...
  getData: function(params, callback) {
    ...
    this.savedFilters = params.filters;
    ...
    [
      ...
      function(find, call) {
        if (!!me.beforeRead) me.beforeRead(params, find, call);
        else call(find);
      },
      ...
      function(find, fields, sort, start, limit, call) {
        ...
        //console.log("\n\nc:", me.collection, " f:", JSON.stringify(find));
  ...
  findData: function(collection, find, fields, sort, start, limit, cb) {
    this.db.getData(
      ...
      (cfg) => {
        this.addTagsToQuery(cfg);
      }
    );
  },

  async addTags(total, data, cb) {
    const ids = data.map((item) => item[this.idField]);
    if (ids.length) {
      const res = await this.src.db
        .collection("tags")
        .findAll({ entity: { $in: ids } }, { entity: 1, tag: 1 });
      if (!res.length) return cb(total, data);
      let tags = {};
      res.forEach((item) => {
        if (!tags[item.entity]) tags[item.entity] = [];
        if (!tags[item.entity].includes(item.tag))
          tags[item.entity].push(item.tag);
      });
      data.forEach((item, i) => {
        if (tags[item[this.idField]]) data[i].tags = tags[item[this.idField]];
      });
    }
    cb(total, data);
  },
  ...
  addTagsToQuery(cfg) {
    let tag;
    for (let f of this.savedFilters) {
      if (f._property == "tags") {
        tag = f._value;
        break;
      }
