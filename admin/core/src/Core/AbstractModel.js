...
Ext.define("Core.AbstractModel", {
  ...
  callModel: function(modelName, attr1, attr2) {
    ...
    if (attr1)
      if (Ext.isFunction(attr1)) callback = attr1;
      else params = attr1;

    if (attr2 && !callback) callback = attr2;

    if (!method[0]) {
      if (callback) callback(null, { mess: "Error method name" });
      return;
    }
  ...
  checkAuthorization: function(params, cb) {
    ...
    [
      ...
      function(_id) {
        me.src.mem.add(token, id, me.config.token.lifetime, function(e, rr) {
          cb(_id);
        });
      }
    ].runEach();
