...
Ext.define("Core.Server", {
  ...
  initProject: function(project, callback) {
    ...
    console.log(
      "Server " + project.serverName + " is listening port " + project.port
    );
  ...
  requestListener: function(req, res) {
    ...
    if (req && req.headers && req.headers.host) {
      ...
      for (var i = 0; i < me.projects.length; i++) {
        if (
          me.projects[i].aliasAction == "redirect" &&
          me.projects[i].aliases &&
          me.projects[i].aliases.indexOf(host) != -1
        ) {
          res.writeHead(301, "Moved Permanently", {
            Location: "http://" + me.projects[i].serverName
          });
          res.end();
          return;
        }
        ...
        if (
          me.projects[i].serverName == "0.0.0.0" ||
          me.projects[i].serverName == host ||
          (me.projects[i].aliasAction != "redirect" &&
            me.projects[i].aliases &&
            me.projects[i].aliases.indexOf(host) != -1)
        ) {
          me.projects[i].server.listener(req, res);
          return;
        }
      }
    }
