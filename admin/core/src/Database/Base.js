...
Ext.define("Database.Base", {
  ...
  collection: function(collection) {
    var me = this,
      name = Object.getPrototypeOf(this).$className.replace(
        /Database$/,
        "Collection"
      );
  ...
  readFieldTypes: function(callback) {
    var me = this,
      className = Object.getPrototypeOf(this).$className.replace(
        /Database$/,
        "fieldtype"
      ),
        ...
        fs.readdir(driverFldDir, function(err, files) {
          if (files) {
            files.each(function(file) {
              ...
              me.fieldTypes[file] = Ext.create(className + "." + file, {
                db: me
              });
            });
          }
          ...
          if (files) {
            files.each(function(file) {
              ...
              if (!me.fieldTypes[file])
                me.fieldTypes[file] = Ext.create("Database.fieldtype." + file, {
                  db: me
                });
            });
          }
  ...
  getSpecificFilter: function(filters, name) {
    if (filters.length) {
      for (var i = 0; i < filters.length; i++) {
        if (filters[i]._property == name) {
          return filters[i];
        }
      }
    }
  ...
  buildWhere: function(params, model, callback) {
    ...
    [
      ...
      function(filters, call) {
        var q = me.getSpecificFilter(filters, "query");
        if (q) {
          find.$or = [];

          Ext.clone(model.fields).prepEach(
            (field, nxt) => {
              if (
                field.filterable &&
                (!params.fieldSet ||
                  params.fieldSet.indexOf(field.name) != -1) &&
                field.type == "string"
              ) {
                var x = me.fieldTypes[field.type].getFilterValue(
                  null,
                  { value: q._value, operator: "like" },
                  field.name,
                  function(cond) {
                    find.$or.push(cond);
                    nxt();
          ...
          /*me.fieldTypes.string.getFilterValue(null, {value: q._value, operator: 'like'}, 'query', function(res) {
                        find.$or = []
      ...
      function(filters, call) {
        ...
        var func = function(i) {
          ...
          for (var j = 0; j < model.fields.length; j++) {
            if (
              model.fields[j].name == filter.property.split(".")[0] ||
              model.fields[j].mapping == filter.property
            ) {
              ...
              fldTp.getFilterValue(model, filter, filter.property, function(
                prm
              ) {
                if (prm) {
                  find.$and.push(prm);
      ...
      function(call) {
        if (!params.showRemoved) find.removed = { $ne: 1 };
        call();
      },
        ...
        for (var i in model.fields) {
          if (
            ...
            if (model.fields[i].type == "ObjectID") {
              find[params.parentField] = me.fieldTypes.ObjectID.getValueToSave(
                model,
                params.parentCode
              );
            } else {
              me.fieldTypes[model.fields[i].type].getFilterValue(
                model,
                { value: params.parentCode, property: model.fields[i].name },
                model.fields[i].name,
                function(v) {
                  var o = {};
