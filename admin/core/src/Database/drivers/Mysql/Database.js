...
Ext.define("Database.drivers.Mysql.Database", {
    ...
    ,createObjectId: function(collection, cb) {
        var me = this;
        this.conn.query("INSERT INTO " + collection + " (_id) VALUES (NULL)", function(e,d) {
            if(d && d.insertId) {
               cb(d.insertId);
               me.conn.query("DELETE FROM " + collection + " WHERE _id=" + d.insertId, function() {})
            } else cb('')
        })
    }
