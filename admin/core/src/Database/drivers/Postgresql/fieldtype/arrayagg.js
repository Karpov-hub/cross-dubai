 ...
 * @class Database.drivers.Postgresql.fieldtype.arrayagg
 * @extend Database.drivers.Mongodb.fieldtype.array
 * arrayagg field.
 */
Ext.define("Database.drivers.Postgresql.fieldtype.arrayagg", {
  extend: "Database.drivers.Mongodb.fieldtype.array",
  ...
  getFilterValue: function(model, filter, name, callback) {
    ...
    if (Ext.isString(filter.value)) {
      if (/^[0-9]{1,}$/.test(filter.value)) {
        f.$or = [{}, {}];
        f.$or[0][name + "___"] = '{' + filter.value + '}';
        f.$or[1][name + "___"] = '{' + filter.value + '}';
      } else f[name + "___"] = '{' + filter.value + '}';
    } else f[name + "___"] = '{' + filter.value + '}';
