    ...
    ,getFilterValue: function(mdl, filter, name, cb) {
        if(
            ...
            if (val[1]) {
              ...
              if (val[1] == "Invalid Date") val.splice(1, 1);
            }
            if (val[0]) {
              val[0] = new Date(Ext.Date.parse(val[0].trim(), "d.m.Y"));
              if (val[0] == "Invalid Date") val.splice(0, 1);
            }

            if (val.length == 2 && val[0] && val[1]) {
              param["$and"] = [{}, {}];
              param["$and"][0][filter.property] = {
                $gte: Ext.Date.format(val[0], "Y-m-d")
              };
              param["$and"][1][filter.property] = {
                $lte: Ext.Date.format(val[1], "Y-m-d")
              };
              ...
              if (operator == "eq") {
                ...
                param["$and"][1][filter.property] = {
                  $lt: Ext.Date.format(
                    new Date(val[0].getTime() + 86400000),
                    "Y-m-d"
                  )
                };
              } else if (operator === "ne") {
                param["$or"] = [{}, {}];
                param["$or"][0][filter.property] = {
                  $gt: Ext.Date.format(
                    new Date(val[0].getTime() + 86400000),
                    "Y-m-d"
                  )
                };
                param["$or"][1][filter.property] = {
                  $lt: Ext.Date.format(val[0], "Y-m-d")
                };
