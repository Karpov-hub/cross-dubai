const sqlBuilder = require("json-sql")({ dialect: "postgresql" });
...
class EnhancedClient extends Client {
  getStartupConf() {
    if (SCHEMA) {
      try {
        const options = { search_path: SCHEMA };
        return {
...
Ext.define("Database.drivers.Postgresql.Database", {
  ...
  connect: function(cfg, callback) {
    ...
    client.connect(async (e, c) => {
      if (e) {
        console.log(e);
        process.exit();
      }
      me.conn = client;
      //if (cfg.schema) await me.selectSchema(cfg.schema);
  ...
  selectSchema(schema) {
    return new Promise((resolve, reject) => {
      this.conn.query(`SET search_path TO '${schema}'`, [], (err, res) => {
        if (err) reject(err);
        else resolve(res);
      });
    });
  ...
  getCollections: function(callback) {
    var me = this;
    me.conn.query(
      `SELECT table_name FROM information_schema.tables WHERE table_schema='${this
        .cfg.schema || "public"}'   AND table_type='BASE TABLE'`,
      [],
      function(err, res) {
        if (res) {
  ...
  queryBuild: function(cfg) {
    ...
    if (cfg.type == "insert" && cfg.values && Object.keys(cfg.values))
      for (let key of Object.keys(cfg.values))
        if (Array.isArray(cfg.values[key]))
          cfg.values[key] = { _arr: cfg.values[key] };

    if (
      cfg.type == "update" &&
      cfg.modifier &&
      cfg.modifier.$set &&
      Object.keys(cfg.modifier.$set)
    )
      for (let key of Object.keys(cfg.modifier.$set))
        if (Array.isArray(cfg.modifier.$set[key]))
          cfg.modifier.$set[key] = { _arr: cfg.modifier.$set[key] };

    const res = sqlBuilder.build(cfg);
    //Search with ilike
  ...
  getData: function(collection, find, fields, sort, start, limit, callback) {
    ...
    [
      // get counts
      function(call) {
        cfg.fields = [{ expression: "count(*) as cnt" }];
