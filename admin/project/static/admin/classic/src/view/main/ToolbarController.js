Ext.define("Admin.view.main.ToolbarController", {
  ...
  readUserProfile: function() {
    ...
    model.readRecord(localStorage.getItem("uid"), function(data) {
      me.view.down("[action=username]").setText(data.fname || data.login);
      if (data.photo) me.view.down("[action=userimage]").setSrc(data.photo);
    });
  }
});
