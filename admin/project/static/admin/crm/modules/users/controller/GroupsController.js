Ext.define("Crm.modules.users.controller.GroupsController", {
  ...
  setControls: function() {
    ...
    this.view.on("beforesave", function(view, data) {
      ...
      data.viewaccess = {};
      view
        .down("[action=view-access]")
        .getStore()
        .each(function(item) {
          data.viewaccess[item.data.name] = {
            show: item.data.show
          };
        });
    });
  ...
  afterDataLoad: function(data, cb) {
    ...
    me.fillModulesStore(data, "model-access", "modelaccess", function() {
      me.fillModulesStore(data, "api-access", "apiaccess", function() {
        me.fillModulesStore(data, "view-access", "viewaccess", function() {
          cb(data);
        });
      });
    });
      ...
      if (data[fieldName]) {
        for (var i = 0; i < models.length; i++) {
          if (data[fieldName][models[i].name]) {
            ...
            models[i].show = data[fieldName][models[i].name].show;
          } else {
            models[i].read = false;
            models[i].add = false;
  ...
  getModels: function(fieldName, callback) {
    ...
    if (fieldName == "modelaccess") {
      me.model.getModules(function(modules) {
        ...
        for (var i = 0; i < modules.length; i++) {
          ...
          else console.log(modules[i].name, D.t(modules[i].name));
        }
        callback(arr);
      });
    }
    ...
    if (fieldName == "viewaccess") {
      me.model.getViews(function(modules) {
        var arr = [],
          name;
        for (var i = 0; i < modules.length; i++) {
          name = D.t(modules[i].view);
          arr.push({
            name: name,
            hname: name,
            dname: modules[i].text + " (" + name + ") "
          });
        }
        callback(arr);
      });
