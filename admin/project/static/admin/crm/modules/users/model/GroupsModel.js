...
Ext.define("Crm.modules.users.model.GroupsModel", {
  ...
  getViews: function(callback) {
    this.runOnServer("getViews", { __CONFIG__ }, callback);
  },

  /* scope:server */
  getArrayViews: function(views, out = []) {
    if (views && views.length) {
      for (const item of views) {
        if (item && item.view) {
          out.push(item);
        } else if (item && item.children) {
          this.getArrayViews(item.children, out);
          // for (const child of item.children) {
          //   if (child && child.view) out.push(child);
          // }
        }
      }
    }
  ...
  /* scope:server */
  $getViews: function(views, cb) {
    return cb(this.getArrayViews(views.__CONFIG__.NavigationTree));
  }
});
