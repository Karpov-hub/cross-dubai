...
Ext.define("Crm.modules.users.model.UsersModel", {
  ...
  /* scope:client */
  getUserViews: function(cb) {
    this.runOnServer("getUserViews", {}, cb);
  },
  /* scope:server */
  $getUserViews: function(data, cb) {
    var me = this;
      ...
      function(u) {
        ...
        me.src.db
          ...
          .toArray(function(e, d) {
            var out = {};
            d.each(function(item) {
              for (var i in item.viewaccess) {
                if (!out[i]) {
                  out[i] = item.viewaccess[i];
                } else {
                  for (var j in item.viewaccess[i]) {
                    if (item.viewaccess[i][j]) {
                      out[i][j] = true;
                    }
                  }
                }
