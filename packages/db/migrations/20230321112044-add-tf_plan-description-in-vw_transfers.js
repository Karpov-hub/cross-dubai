      ...
      t.data ->> 'merchant'::text AS merchant_id,
          CASE
              ...
              CASE t.status
                  ...
                  WHEN 4 THEN 'TRANSFERRED'::text
                  ELSE NULL::text
              END
              WHEN t.held IS NOT TRUE AND t.canceled IS NOT TRUE THEN 'TRANSFERRED'::text
              WHEN t.held IS TRUE AND t.canceled IS NOT TRUE THEN 'PENDING'::text
              ELSE NULL::text
          END AS string_status,
      t.status,
      t.invisibility_exp_date,
      tp.description tf_by_plan_description
     FROM realms r,
      transfers t
       ...
       LEFT JOIN transfers_plans tp on tp.id = (t.data ->> 'pipeline')::uuid
