  ...
  down: (queryInterface, Sequelize) => {
    return queryInterface.sequelize.transaction((t) => {
      return Promise.all([
        ...
        where 
          t.plan_transfer_id=p.id and t.id=x.transfer_id 
        group by p.id, x.acc_src, x.acc_dst, p.ctime, p.step, t.event_name, x.held, x.canceled, t.show_to_client, t.invisibility_exp_date, t.data
        union 
        select 
          t.id, 
          x.acc_src, 
        ...
        where 
          plan_transfer_id is null and t.id=x.transfer_id 
        group by t.id, x.acc_src, x.acc_dst, t.ctime, t.event_name, x.held, x.canceled, t.show_to_client, t.invisibility_exp_date, t.data`,
          {
            transaction: t
          }
        )
