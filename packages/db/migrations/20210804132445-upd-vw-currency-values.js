    ...
    return queryInterface.sequelize.transaction((t) => {
      return Promise.all([
        ...
        queryInterface.sequelize.query(
          `create view vw_currency_values as select v.abbr, realm, amount, value, c.crypto, c.decimal, c.withdrawal_decimal from (select id, jsonb_array_elements_text(realms -> '_arr'::text) AS realm from currency_history where active=true) as h, currency_values v, currency c where c.abbr=v.abbr and h.id=v.pid`,
          { transaction: t }
        )
      ]);
    });
