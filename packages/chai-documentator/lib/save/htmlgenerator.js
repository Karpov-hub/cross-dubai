...
function checkServiceMethodInCode(service, method, code) {
  const re1 = new RegExp(`method:[ ]{0,}"${method}`);
  const re2 = new RegExp(`service:[ ]{0,}"${service}`);

  return re1.test(code) && re2.test(code);
}
...
function readParsedTest(arr, reqres) {
  arr.forEach((item) => {
    if (item.type == "it") {
      if (reqres[itIndex]) {
        let service = reqres[itIndex].request.body.header.service;
        let method = reqres[itIndex].request.body.header.method;
        if (checkServiceMethodInCode(service, method, item.code)) {
  ...
  service.methods.forEach((method) => {
    ...
    swagger.paths[path] = {
      post: {
        tags: [service.name],
        summary: data.title,
        description: data.comments.join("<br>"),
